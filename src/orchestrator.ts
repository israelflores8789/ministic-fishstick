	...
	private async _startWatcher(): Promise<void> {
		...
		try {
			...
			this._fileWatcherSubscriptions = [
				...
				this.fileWatcher.onBatchProgressUpdate(({ processedInBatch, totalInBatch, currentFile }: { processedInBatch: number; totalInBatch: number; currentFile?: string }) => {
		...
		try {
			...
			if (hasExistingData && !collectionCreated) {
				...
				const result = await this.scanner.scanDirectory(
					this.workspacePath,
					(batchError: Error) => {
						console.error(`[CodeIndexOrchestrator] Error during incremental scan batch: ${batchError.message}`)
				...
				const result = await this.scanner.scanDirectory(
					this.workspacePath,
					(batchError: Error) => {
						console.error(`[CodeIndexOrchestrator] Error during initial scan batch: ${batchError.message}`)
						batchErrors.push(batchError)
					},
					handleBlocksIndexed,
					handleFileParsed,
			...
			this.stateManager.setSystemState("Error", `Failed during scan: ${error.message || "Unknown error"}`)
			this.stopWatcher()
		} finally {
