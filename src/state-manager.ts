...
export class CodeIndexStateManager {
  ...
  public reportBlockIndexingProgress(processedItems: number, totalItems: number): void {
    const progressChanged =
      processedItems !== this._processedItems || totalItems !== this._totalItems
    ...
    if (progressChanged || this._systemStatus !== 'Indexing') {
      ...
      if (
        oldStatus !== this._systemStatus ||
        oldMessage !== this._statusMessage ||
        progressChanged
      ) {
        this._emitter.emit('progress', this.getCurrentStatus())
      }
  ...
  public reportFileQueueProgress(
    ...
    const progressChanged =
      processedFiles !== this._processedItems || totalFiles !== this._totalItems
